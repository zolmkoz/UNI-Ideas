...
function smiley_full(&$data)
{
	...
	switch ($mode) {
		case 'm':
			...
			while ($r = db_rowobj($c)) {
				...
				if ($format == 'rdf') {
					$feed_header .= '<rdf:li rdf:resource="http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'" />';
	...
	<content:item rdf:about="poll_opt">
		...
		<opt_votes>'.$o[1].'</opt_votes>
					...
					$feed_data .= ($basic ? '
<item rdf:about="http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'">
	<title>'.htmlspecialchars($r->subject).'</title>
	<link>http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'</link>
...
' ) ;
	...
	<title>'.htmlspecialchars($r->subject).'</title>
	<link href="http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'" />
	<id>http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;goto='.$r->id.'&amp;th='.$r->thread_id.'#msg_'.$r->id.'</id>
...
' ) .'
<channel rdf:about="http://localhost/uni-ideas/index.php">
	<title>'.$FORUM_TITLE.' - RDF feed</title>
	<link>http://localhost/uni-ideas/index.php</link>
...
<feed xmlns="http://www.w3.org/2005/Atom">
	...
	<link href="http://localhost/uni-ideas/index.php" />
	<updated>'.gmdate('Y-m-d\TH:i:s', __request_timestamp__).'-00:00</updated>
	<id>http://localhost/uni-ideas/index.php</id>
	...
	'.$feed_data.'
		...
		case 't':
			...
			while ($r = db_rowobj($c)) {
				...
				if ($format == 'rdf') {
					$feed_header .= '<rdf:li rdf:resource="http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;th='.$r->id.'" />';
					$feed_data .= ($basic ? '
<item rdf:about="http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;th='.$r->id.'">
	<title>'.htmlspecialchars($r->subject).'</title>
	<link>http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;th='.$r->id.'</link>
	<description>'.sp($body).'</description>
...
' ) ;
	...
	<link href="http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;th='.$r->id.'" />
	<id>http://localhost/uni-ideas/index.phpindex.php?t=rview&amp;th='.$r->id.'</id>
...
' : '' ) .'
	...
	<link>http://localhost/uni-ideas/index.phpindex.php?t=usrinfo&amp;id='.$r->id.'</link>
	<author>'.sp($r->name).'</author>
	'.($r->last_visit ? '<pubDate>'.gmdate('Y-m-d\TH:i:s', $r->last_visit).'</pubDate>' : '' ) .'
</item>';
				if ($format == 'atom') $feed_data .= '<entry>
	<title>'.sp($r->alias).'</title>
	<link href="http://localhost/uni-ideas/index.phpindex.php?t=usrinfo&amp;id='.$r->id.'" />
	<id>http://localhost/uni-ideas/index.phpindex.php?t=usrinfo&amp;id='.$r->id.'</id>
	<author>
		<name>'.sp($r->name).'</name>
