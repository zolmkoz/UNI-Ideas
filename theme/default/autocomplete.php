...
if (_uid === '_uid') {
	...
	if (!empty($_GET['lookup']) && !empty($_GET['term'])) {
		...
		$c = uq(q_limit('SELECT '. $lookup .' FROM fud30_users WHERE '. $lookup .' LIKE '. $term .' AND '. q_bitand('users_opt', 1073741824) .'= 0', 10));
		$rows = array();
		while ($r = db_rowarr($c)) {
	...
	if (!empty($_GET['search']) && !empty($_GET['term'])) {
		...
		$c = uq(q_limit('SELECT DISTINCT subject FROM fud30_thread t LEFT JOIN fud30_msg m on t.root_msg_id = m.id WHERE subject LIKE '. $term, 10));
		$rows = array();
		while ($r = db_rowarr($c)) {
			$rows[] = array('value' => $r[0]);
		}
	...
	if (!empty($_GET['check']) && !empty($_GET['term'])) {
		...
		if (q_singleval('SELECT '. $lookup .' FROM fud30_users WHERE '. $lookup .' = '. _esc($term))) {
