...
/* Print number of unread private messages in User Control Panel. */
	...
	if ($del) {
		...
		if (empty($_POST['confirm'])) {
			if ($data[2] != $data[4]) {
				$delete_msg = 'You are about to <span class="ErrorText">delete</span> the message titled: <b>'.$data[3].'</b>';
			} else {
				$delete_msg = 'You are about to <span class="ErrorText">delete</span> the <span class="ErrorText">entire topic</span> titled: <b>'.$data[3].'</b>';
			}
			?> <!DOCTYPE html>
<html lang="en" dir="ltr">
<head>
...
<tr>
	<th>Confirm Topic/Message Deletion</th>
</tr>
<tr class="RowStyleA ac">
	<td class="GenText">
		<form method="post" action="/uni-ideas/index.php?t=mmod">
			...
			<div class="mnavWarnTbl">
				...
				<br />
				<label><input type="checkbox" name="del_inc_body" /> Include the body of the removed message that will be sent to the author.</label>
				<br /><br />
			</div>
			<br />
...
'.$body;
			...
			if ($data[2] == $data[4]) {
				...
				if (strpos($usr->returnto, 'selmsg') === false) {
					if ($FUD_OPT_2 & 32768) {
						header('Location: /uni-ideas/index.php/f/'. $data[0] .'/'. _rsidl);
					} else {
						header('Location: /uni-ideas/index.php?t='. t_thread_view .'&'. _rsidl .'&frm_id='. $data[0]);
					}
					exit;
				} else {
					check_return($usr->returnto);
		...
		if (d_thread_view == 'tree') {
			if (!$data[5]) {
				if ($FUD_OPT_2 & 32768) {
					header('Location: /uni-ideas/index.php/mv/tree/'. $data[1] .'/'. _rsidl);
				} else {
					header('Location: /uni-ideas/index.php?t=tree&'. _rsidl .'&th='. $data[1]);
				}
			} else {
				if ($FUD_OPT_2 & 32768) {
					header('Location: /uni-ideas/index.php/mv/tree/'. $data[1] .'/'. $data[5] .'/'. _rsidl);
				} else {
					header('Location: /uni-ideas/index.php?t=tree&'. _rsidl .'&th='. $data[1] .'&mid='. $data[5]);
				}
			}
		} else {
			...
			if ($FUD_OPT_2 & 32768) {
				header('Location: /uni-ideas/index.php/mv/msg/'. $data[1] .'/'. $prev_id .'/'. _rsidl);
			} else {
				header('Location: /uni-ideas/index.php?t=msg&th='. $data[1] .'&'. _rsidl .'&goto='. $prev_id);
			}
		}
		exit;
