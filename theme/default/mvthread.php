...
<tr>
	...
	if ($to) {
		...
		if ($FUD_OPT_2 & 32768 && !empty($_SERVER['PATH_INFO'])) {
			exit('<html><script>window.opener.location=\'/uni-ideas/index.php/f/'. $thr->forum_id .'/'. _rsid .'\'; window.close();</script></html>');
		} else {
			exit('<html><script>window.opener.location=\'/uni-ideas/index.php?t='. t_thread_view .'&'. _rsid .'&frm_id='. $thr->forum_id .'\'; window.close();</script></html>');
		}
	}
	...
	<td class="mvTc" style="padding-left: '.($tabw = ($i[0] * 10 + 2)).'px">'.$i[1].'</td>
			...
			if ($r[3] || $is_a || $r[4] & 8192) {
				$table_data .= '<tr>
	<td style="padding-left: '.$tabw.'px"><a href="/uni-ideas/index.php?t=mvthread&amp;th='.$thr->id.'&amp;to='.$r[1].'&amp;'._rsid.'&amp;SQ='.$GLOBALS['sq'].'">'.$r[0].'</a></td>
</tr>';
			} else {
				$table_data .= '<tr>
	<td style="padding-left: '.$tabw.'px">'.$r[0].' [<a href="/uni-ideas/index.php?t=mvthread&amp;th='.$thr->id.'&amp;'._rsid.'&amp;thx='.$r[1].'&amp;SQ='.$GLOBALS['sq'].'">request a move</a>]</td>
</tr>';
			}
		}
		unset($c);
...
<body>
<div class="content">
<form action="/uni-ideas/index.php?t=mvthread" id="mvthread" method="post">
<table cellspacing="0" cellpadding="3" class="wa dashed">
<tr>
	<td class="small">Move topic <b><?php echo $thr->subject; ?></b> to:</td>
</tr>
<?php echo $table_data; ?>
</table>
<?php echo _hs; ?>
