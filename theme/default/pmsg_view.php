...
<tr>
	...
	if (($nid = q_singleval(q_limit('SELECT p.id FROM fud30_pmsg p INNER JOIN fud30_users u ON u.id=p.ouser_id WHERE p.duser_id='. _uid .' AND p.fldr='. $m->fldr .' AND post_stamp>'. $m->post_stamp .' ORDER BY p.post_stamp ASC', 1)))) {
		$dpmsg_next_message = '<a href="/uni-ideas/index.php?t=pmsg_view&amp;'._rsid.'&amp;id='.$nid.'">Next message <img src="/uni-ideas/theme/default/images/goto.gif" alt="" /></a>';
	} else {
		$dpmsg_next_message = '';
	}
	...
	if (($pid = q_singleval(q_limit('SELECT p.id FROM fud30_pmsg p INNER JOIN fud30_users u ON u.id=p.ouser_id WHERE p.duser_id='. _uid .' AND p.fldr='. $m->fldr .' AND p.post_stamp<'. $m->post_stamp .' ORDER BY p.post_stamp DESC', 1)))) {
		$dpmsg_prev_message = '<a href="/uni-ideas/index.php?t=pmsg_view&amp;'._rsid.'&amp;id='.$pid.'"><img src="/uni-ideas/theme/default/images/goback.gif" alt="" /> Previous message</a>';
	} else {
		$dpmsg_prev_message = '';
	}
	...
	if (!$m->read_stamp && $m->pmsg_opt & 16) {
		...
		if ($m->ouser_id != _uid && $m->pmsg_opt & 4 && !isset($_GET['dr'])) {
			...
			$track_msg->body = 'Hello,<br />'.$usr->login.' has opened your private message titled, "'.$m->subject.'".<br />This is an automated notification generated at '.utf8_encode(strftime('%a, %d %B %Y %H:%M', __request_timestamp__)).'<br />';
			$track_msg->add(1);
		}
	}
