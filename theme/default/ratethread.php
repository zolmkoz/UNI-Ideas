...
if (_uid === '_uid') {
	...
	if (isset($_GET['rate_thread_id'], $_GET['sel_vote']) && ($rt = (int) $_GET['sel_vote'])) {
		...
		if (db_li('INSERT INTO fud30_thread_rate_track (thread_id, user_id, stamp, rating) VALUES('. $th .', '. _uid .', '. __request_timestamp__ .', '. $rt .')', $ef)) {
			...
			exit('&nbsp;('.($MOD ? '<a href="javascript://" onclick="window_open(\'/uni-ideas/index.php?t=ratingtrack&amp;'._rsid.'&amp;th='.$frm->id.'\', \'th_rating_track\', 300, 400);">' : '' )  .'<img src="/uni-ideas/theme/default/images/'.$frm->rating.'stars.gif" title="'.$frm->rating.' from '.convertPlural($frm->n_rating, array(''.$frm->n_rating.' vote',''.$frm->n_rating.' votes')).'" alt=""/>'.($MOD ? '</a>' : '' )  .') '.convertPlural($frm->n_rating, array(''.$frm->n_rating.' Vote',''.$frm->n_rating.' Votes')).'');
		}
	}
