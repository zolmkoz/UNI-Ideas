...
/* main */
	...
	if (!__fud_real_user__ && !isset($_POST['reg_coppa']) && !isset($_GET['reg_coppa'])) {
		if ($FUD_OPT_1 & 1048576) {
			if ($FUD_OPT_2 & 32768) {
				header('Location: /uni-ideas/index.php/cp/'. _rsidl);
			} else {
				header('Location: /uni-ideas/index.php?t=coppa&'. _rsidl);
			}
		} else {
			if ($FUD_OPT_2 & 32768) {
				header('Location: /uni-ideas/index.php/pr/0/'. _rsidl);
			} else {
				header('Location: /uni-ideas/index.php?t=pre_reg&'. _rsidl);
			}
		}
		exit;
	}
	...
	if (isset($_POST['fud_submit']) && !isset($_POST['btn_detach']) && !isset($_POST['btn_upload']) && !register_form_check($uent->id)) {
		...
		if (!__fud_real_user__) { /* new user */
			...
			if ($FUD_OPT_1 & 1048576 && $uent->users_opt & 262144) {
				if ($FUD_OPT_2 & 32768) {
					header('Location: /uni-ideas/index.php/cpf/'. _rsidl);
				} else {
					header('Location: /uni-ideas/index.php?t=coppa_fax&'. _rsidl);
				}
				exit;
			} else if (!($uent->users_opt & 131072) || $FUD_OPT_2 & 1024) {
				header('Location: /uni-ideas/index.php'. ($FUD_OPT_2 & 32768 ? '/rc/' : '?t=reg_conf&') . _rsidl);
				exit;
			}
			...
			if ($FUD_OPT_1 & 28 && _uid) {
				if ($_POST['avatar_type'] == 'b') { /* built-in avatar */
					...
					if ($_POST['reg_avatar'] == '0') {
						...
						$uent->avatar_loc = make_avatar_loc('images/avatars/'. $img, $WWW_ROOT_DISK, '/uni-ideas/');
					...
					if ($common_av_name) {
						...
						if (($uent->avatar_loc = make_avatar_loc($av_path, $WWW_ROOT_DISK, '/uni-ideas/'))) {
			...
			if ($a_type) { /* Rare condition, no built-in avatars & no other avatars are allowed. */
				...
				if (isset($_POST['prev_loaded'])) {
					if ((!empty($_POST['reg_avatar']) && $_POST['reg_avatar'] == $uent->avatar) || (!empty($avatar_arr['file']) && empty($avatar_arr['del']) && $avatar_arr['leave'])) {
						...
						$custom_avatar_preview = make_avatar_loc('images/avatars/'. $im, $WWW_ROOT_DISK, '/uni-ideas/');
					} else {
						...
						$custom_avatar_preview = $common_name ? make_avatar_loc(basename($common_name), $TMP, '/uni-ideas/?t=tmp_view&img=') : '';
					}
...
<script>
	...
	$(document).ready(function() {
            ...
            $("#reg_login").change(function() {
                 $("#reg_login-result").html("<img src='/uni-ideas/theme/default/images/ajax-loader.gif' />");
 
                 $.ajax({
                    url:"index.php?t=autocomplete&check=login&term=" + $("#reg_login").val(),
                    success:function(data) {
                        if (data==1) {
                            $("#reg_login-result").html("<img src='/uni-ideas/theme/default/images/tick.png' />Available");
                        } else {
                            $("#reg_login-result").html("<img src='/uni-ideas/theme/default/images/cross.png' /> Already taken");
                        }
                    }
                 });
            });
