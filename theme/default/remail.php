...
/* Print number of unread private messages in User Control Panel. */
	...
	if (isset($_POST['posted']) && _uid && !check_femail_form()) {
		...
		error_dialog('E-mail was sent', 'The e-mail to your friend at '.filter_var($_POST['femail'], FILTER_SANITIZE_STRING).' about the '.$data->subject.' topic was successfully sent.');
...
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText nw vt SmallText">Subject:<br /><i>required</i></td>
...
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText nw vt">Message:<span class="SmallText"><br /><i>required</i></span></td>
	<td valign="top" class="nw"><textarea name="body" rows="19" cols="78" wrap="PHYSICAL">'.(isset($_POST['body']) ? filter_var($_POST['body'], FILTER_SANITIZE_STRING).'' : 'Hello,\n\nThere is an interesting topic about "'.$data->subject.'" on '.$GLOBALS['FORUM_TITLE'].' forum that you may want to read. You can see the topic at:\n http://localhost/uni-ideas/index.php?t=rview&amp;th='.$data->id.'&amp;rid='._uid.'\n\nYour friend,\n\n'.$usr->alias.'\n' ) .'</textarea>'.get_err('body').'</td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText ar" colspan="2"><input type="submit" class="button" name="submit" value="Send E-mail Now" /></td>
</tr>' : '<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText ac SmallText">Copy this message into a mail client of your choice to send it to your friend(s).</td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText"><textarea name="body" rows="19" cols="78">'.(isset($_POST['body']) ? filter_var($_POST['body'], FILTER_SANITIZE_STRING).'' : 'Hello,\n\nThere is an interesting topic about "'.$data->subject.'" on '.$GLOBALS['FORUM_TITLE'].' forum that you may want to read. You can see the topic at:\n http://localhost/uni-ideas/index.php?t=rview&amp;th='.$data->id.'&amp;rid='._uid.'\n\nYour friend,\n\n'.$usr->alias.'\n' ) .'</textarea></td>
</tr>
<tr class="'.alt_var('page_alt','RowStyleA','RowStyleB').'">
	<td class="GenText ar"><input type="submit" class="button" name="done" value="Done" /></td>
</tr>';
...
<form action="/uni-ideas/index.php?t=remail" id="remail" method="post">
<input type="hidden" name="posted" value="1" />
<table cellspacing="1" cellpadding="2" class="MiniTable">
<tr>
	<th colspan="2">E-mail This Topic to a Friend</th>
</tr>
