...
function trim_body($body)
{
	...
	$TITLE_EXTRA = ': Search for '.filter_var($srch, FILTER_SANITIZE_STRING);
...
<tr>
	<th class="wa ac">No Results</th>
			...
			if ($FUD_OPT_1 & 2097152 && extension_loaded('enchant') && $usr->pspell_lang) {
				...
				if (enchant_broker_dict_exists($r, $usr->pspell_lang)) {
					...
					if (!empty($srch_links)) {
						$search_data .=  '<div class="wa">Do you mean: '. $srch_links . '</div>';
					}
				}
			}
	...
	<td class="'.alt_var('search_alt','RowStyleA','RowStyleB').'">
		<b>Forum</b>: <a href="/uni-ideas/index.php?t='.t_thread_view.'&amp;frm_id='.$r->forum_id.'&amp;'._rsid.'">'.$r->forum_name.'</a> &laquo;&raquo;
		<b>Posted on</b>: <span class="DateText">'.utf8_encode(strftime('%a, %d %B %Y %H:%M', $r->post_stamp)).'</span> &laquo;&raquo;
		<b>By:</b> '.(!empty($r->poster_id) ? '<a href="/uni-ideas/index.php?t=usrinfo&amp;id='.$r->poster_id.'&amp;'._rsid.'">'.filter_var($r->alias, FILTER_SANITIZE_STRING).'</a>' : $GLOBALS['ANON_NICK'].'' ) .'<br />
		<span class="MsgSubText"><a href="/uni-ideas/index.php?t='.d_thread_view.'&amp;goto='.$r->id.'&amp;'._rsid.'&amp;srch='.urlencode($srch).'#msg_'.$r->id.'">'.$r->subject.'</a></span><br />
		<span class="MsgBodyText">
			'.trim_body(read_msg_body($r->foff, $r->length, $r->file_id)).'
		</span>
		'.(($is_a || $r->md) ? '
		<div class="ModOpt">Moderator Options: <a href="/uni-ideas/index.php?t=mmod&amp;'._rsid.'&amp;th='.$r->thread_id.'&amp;del='.$r->id.'">Delete</a>
'.($r->is_rootm ? '
 | <a href="javascript://" onclick="window_open(\'/uni-ideas/index.php?t=mvthread&amp;'._rsid.'&amp;th='.$r->thread_id.'\', \'th_move\', 300, 400);">Move</a> | <a href="/uni-ideas/index.php?t=mmod&amp;'._rsid.'&amp;th='.$r->thread_id.'&amp;'.($r->is_lckd ? 'unlock' : 'lock' )  .'=1&amp;SQ='.$GLOBALS['sq'].'">'.($r->is_lckd ? 'Unlock Topic' : 'Lock Topic' )  .'</a>
' : '' )  .'
</div>
		' : '' )  .'
	</td>
...
'.$search_data.'
			...
			if ($FUD_OPT_2 & 32768) {	// USE_PATH_INFO?
				$page_pager = tmpl_create_pager($start, $ppg, $total, '/uni-ideas/index.php/s/'. urlencode($srch) .'/'. $field .'/'. $search_logic .'/'. $sort_order .'/'. ($forum_limiter ? $forum_limiter : 0) .'/', '/'. urlencode($author) .'/'. _rsid);
			} else {
				$page_pager = tmpl_create_pager($start, $ppg, $total, '/uni-ideas/index.php?t=search&amp;srch='. urlencode($srch) .'&amp;field='. $field .'&amp;'. _rsid .'&amp;search_logic='. $search_logic .'&amp;sort_order='. $sort_order .'&amp;forum_limiter='. $forum_limiter .'&amp;author='. urlencode($author));
			}
		}
	} else {
...
<form id="search" method="get" action="/uni-ideas/index.php">
<?php echo _hs; ?>
<input type="hidden" name="t" value="search" />
<table cellspacing="1" cellpadding="2" class="ContentTable">
<tr>
...
<tr class="RowStyleA">
	...
	<td class="RowStyleB SmallText">
		...
		<div class="sr">Filter by user:<br /><input class="SmallText" type="text" value="<?php echo filter_var($author, FILTER_SANITIZE_STRING); ?>" name="author" /></div>
	</td>
</tr>
</table>
</form>
