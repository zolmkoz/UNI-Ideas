...
function path_info_lnk($var, $val)
{
	...
	ses_update_status($usr->sid, 'Browsing <a href="/uni-ideas/index.php?t=selmsg&amp;date=today&amp;rid='.$usr->id.'">Today&#39;s Posts</a>');
...
if (_uid) {
	...
	if ($unread_limit || $total) {
		...
		while ($r = db_rowobj($c)) {
			if ($prev_frm != $r->forum_id) {
				...
				$message_data .= '<tr>
	<th class="SelFS">
		Forum: <a class="thLnk" href="/uni-ideas/index.php?t='.t_thread_view.'&amp;frm_id='.$r->forum_id.'&amp;'._rsid.'"><span class="lg">'.$r->name.'</span></a>
	</th>
</tr>';
			...
			if ($prev_th != $r->thread_id) {
				...
				$message_data .= '<tr>
	<th class="SelTS">
		&nbsp;Topic: <a class="thLnk" href="/uni-ideas/index.php?t='.d_thread_view.'&amp;goto='.$r->id.'&amp;'._rsid.'#msg_'.$r->id.'">'.$r->thr_subject.'</a>
	</th>
</tr>';
			}
	...
	if (!$unread_limit && $total > $count) {
		...
		if ($FUD_OPT_2 & 32768 && isset($_SERVER['PATH_INFO'])) {
			...
			$pager = tmpl_create_pager($start, $count, $total, '/uni-ideas/index.php'. $p .'start/', '/'. _rsid);
		} else {
			$pager = tmpl_create_pager($start, $count, $total, '/uni-ideas/index.php?'. str_replace('&amp;start='. $start, '', $_SERVER['QUERY_STRING']));
		}
		...
		$pager = $message_data ? '<div class="GenText ac">[<a href="/uni-ideas/index.php?'.$_SERVER['QUERY_STRING'].'" title="Show more unread messages and mark the currently viewable messages read">more unread messages</a>]</div><img src="/uni-ideas/blank.gif" alt="" height="3" />' : '';
	} else {
		$pager = '';
	}
...
<a href="/uni-ideas/index.php?<?php echo $dt_opt; ?>" title="Toggle today&#39;s messages filter (will only show messages made today)">Today&#39;s Messages <?php echo (isset($_GET['date']) ? '<span class="selmsgInd">(<span class="GenTextRed">on</span>)</span>' : '<span class="selmsgInd">(off)</span>' )  .'</a>
'.(_uid ? '&nbsp;| <a href="/uni-ideas/index.php?'.$un_opt.'" title="Toggle unread messages filter (will only show unread messages)">Unread Messages '.(isset($_GET['unread']) ? '<span class="selmsgInd">(<span class="GenTextRed">on</span>)</span>' : '<span class="selmsgInd">(off)</span>' )  .'</a>' : ''); ?>
<?php echo (_uid ? '&nbsp;| <a href="/uni-ideas/index.php?'.$frm_opt.'" title="Toggle the subscribed forums filter (will only show messages from the forums you are subscribed to)">Subscribed Forums '.(isset($_GET['sub_forum_limit']) ? '<span class="selmsgInd">(<span class="GenTextRed">on</span>)</span>' : '<span class="selmsgInd">(off)</span>' )  .'</a>' : ''); ?>
<?php echo (_uid ? '&nbsp;| <a href="/uni-ideas/index.php?'.$th_opt.'" title="Toggle the subscribed topics filter (will only show messages from the topics you are subscribed to)">Subscribed Topics '.(isset($_GET['sub_th_limit']) ? '<span class="selmsgInd">(<span class="GenTextRed">on</span>)</span>' : '<span class="selmsgInd">(off)</span>' )  .'</a>' : ''); ?>
&nbsp;| <a href="/uni-ideas/index.php?<?php echo $rp_opt; ?>" title="Toggle the un-answered messages filter (will show only messages that have no replies)">Unanswered Messages <?php echo (isset($_GET['reply_count']) ? '<span class="selmsgInd">(<span class="GenTextRed">on</span>)</span>' : '<span class="selmsgInd">(off)</span>'); ?></a>
