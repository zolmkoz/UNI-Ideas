...
<tr>
	...
	ses_update_status($usr->sid, 'Viewing posts by: <a href="/uni-ideas/index.php?t=usrinfo&amp;id='.$uid.'">'.$u_alias.'</a>');
	...
	<td class="DateText nw">'.utf8_encode(strftime('%a, %d %B %Y %H:%M', $r[4])).'</td>
		...
		if ($FUD_OPT_2 & 32768) {
			$pager = tmpl_create_pager($start, $THREADS_PER_PAGE, $total, '/uni-ideas/index.php/sp/'. $uid .'/'. $SORT_ORDER .'/', '/'. _rsid);
		} else {
			$pager = tmpl_create_pager($start, $THREADS_PER_PAGE, $total, '/uni-ideas/index.php?t=showposts&amp;id='. $uid .'&amp;so='. $SORT_ORDER .'&amp;'. _rsid);
		}
	}
