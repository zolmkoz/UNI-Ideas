...
function th_frm_last_post_id($id, $th)
{
	...
	if ($forum && !empty($_POST['new_title']) && is_string($_POST['new_title']) && !empty($_POST['sel_th']) && is_array($_POST['sel_th'])) {
		...
		if ($FUD_OPT_2 & 32768) {
			header('Location: /uni-ideas/index.php/t/'. $th_id .'/'. _rsidl);
		} else {
			header('Location: /uni-ideas/index.php?t='. d_thread_view .'&th='. $th_id .'&'. _rsidl);
		}
		exit;
	}
...
<form id="split_th" action="/uni-ideas/index.php?t=split_th" method="post">
<?php echo _hs; ?>
