...
function make_perms_query(&$fields, &$join, $fid='')
{
	...
	ses_update_status($usr->sid, 'Browsing forum (tree view) <a href="/uni-ideas/index.php?t=threadt&amp;frm_id='.$frm->id.'">'.$frm->name.'</a>', $frm->id);
...
function draw_forum_path($cid, $fn='', $fid=0, $tn='')
{
							...
							if ($TREE_THREADS_MAX_DEPTH > $lev) {
								...
								if (_uid) {
									if ($usr->last_read < $cur->post_stamp && $cur->post_stamp>$cur->last_view) {
										$thread_read_status = $cur->thread_opt & 1 ? '<img src="/uni-ideas/theme/default/images/unreadlocked.png" title="Locked topic with unread messages" alt="" width="22" height="22"  />'	: '<img src="/uni-ideas/theme/default/images/unread.png" title="This topic contains messages you have not yet read" alt="" width="22" height="22" />';
									} else {
										$thread_read_status = $cur->thread_opt & 1 ? '<img src="/uni-ideas/theme/default/images/readlocked.png" title="This topic has been locked" alt="" width="22" height="22" />' : '<img src="/uni-ideas/theme/default/images/read.png" title="This topic has no unread messages" alt="" width="22" height="22" />';
									}
								} else {
									$thread_read_status = $cur->thread_opt & 1 ? '<img src="/uni-ideas/theme/default/images/readlocked.png" title="This topic has been locked" alt="" width="22" height="22" />' : '<img src="/uni-ideas/theme/default/images/read.png" title="The read &amp; unread messages are only tracked for registered users" alt="" width="22" height="22" />';
								}

								$thread_list_table_data .= '<tr>
	<td class="RowStyleB">'.$thread_read_status.'</td>
	<td class="RowStyleB">'.($cur->icon ? '<img src="/uni-ideas/images/message_icons/'.$cur->icon.'" alt="'.$cur->icon.'" />' : '&nbsp;' ) .'</td>
	<td title="'.$cur->tdescr.'" class="tt" style="padding-left: '.(20 * ($lev - 1)).'px">
		'.($cur->poll_id ? 'Poll:&nbsp;' : '' ) .'
		'.($cur->attach_cnt ? '<img src="/uni-ideas/theme/default/images/attachment.gif" alt="" />' : '' ) .'
		<a href="/uni-ideas/index.php?t='.d_thread_view.'&amp;goto='.$cur->id.'&amp;'._rsid.'#msg_'.$cur->id.'" class="big">'.$cur->subject.'</a>
	...
	<td class="tt" style="padding-left: '.($width += 20).'px"><a href="/uni-ideas/index.php?t='.d_thread_view.'&amp;goto='.$cur->id.'&amp;'._rsid.'#msg_'.$cur->id.'" class="big">&lt;more&gt;</a></td>
</tr>';
							}
	...
	if ($FUD_OPT_2 & 32768) {
		$page_pager = tmpl_create_pager($start, 1, ceil($frm->thread_count / $THREADS_PER_PAGE), '/uni-ideas/index.php/sf/threadt/'. $frm->id .'/1/', '/'. _rsid);
	} else {
		$page_pager = tmpl_create_pager($start, 1, ceil($frm->thread_count / $THREADS_PER_PAGE), '/uni-ideas/index.php?t=threadt&amp;frm_id='. $frm->id .'&amp;'. _rsid);
	}
